"""
Mock-sim A/B: REV1 vs V5.1, in the same domain the execution_model curves were fit from.

REV1 = V5.1 + Curve 1 (per-leg-length ceiling bins) ONLY, with a 0.5s ceiling-hold
...
Run:
  python3.13 mock_ab_rev1_vs_v51.py
"""
...
LOG_PATH     = os.path.join(BASE_DIR, "logs/mock_ab_rev1_vs_v51.json")
TRIALS_PER_COMBO = 3
MAVSDK_PORT  = 14540
...
class V51Planner:
    ...
    # Hook V5.1 uses scalar ceiling; REV1 overrides this to use per-leg bins.
    def _ceiling(self, state, target, dxy):
        return self.px4_speed_ceiling


# =====================================================================
# REV1 Planner — V5.1 + Curve 1 only (per-leg ceiling bins) + ceiling
# smoothing. Curve 2 (prescriptive launch ramp) is dropped because it
# was descriptive of V5.1's achieved speed and feeding it back as a
# command target throttled the launch (see mock_ab_exm1_vs_v51 results).
#
# Smoothing: after each gate pass, hold the MAX of the previous and
# new bin ceiling for HOLD_SECS. This prevents an immediate ceiling
# drop on entering a shorter leg, which forces the drone to bleed
# speed and then rebuild it (the exit_rebuild_mps2 loss bucket).
# =====================================================================
class Rev1Planner(V51Planner):
    _CEILING_BINS = [
        (0,   8,  7.21),
        (8,  12,  7.81),
        (12, 16,  8.34),
    ...
    def _ceiling(self, state, target, dxy):
        new_c = self._bin_ceiling(self.current_leg_length)
    ...
    def on_gate_passed(self, speed):
        # Snapshot the ceiling we were *just* using before recomputing for the new leg.
        self._prev_ceiling = self._bin_ceiling(self.current_leg_length)
    ...
    def plan(self, state, target, next_gate=None):
        ...
        # entirely to V5.1 (which calls self._ceiling for the bin lookup).
        if self._new_leg:
...
async def main():
    ...
    configs=[
        ...
        ('REV1', Rev1Planner, dict(max_speed=11.0, cruise_speed=9.0, base_blend=1.5)),
    ]
    ...
    # ---- Summary ----
    print(f'\n\n{"="*80}')
    print('REV1 vs V5.1 HEAD-TO-HEAD on MOCK SIM  (median over trials, OK only)')
    print(f'{"="*80}')
    ...
    for cn in ['mixed','sprint','technical']:
        ...
        a=summary.get((cn,'V5.1')); b=summary.get((cn,'REV1'))
        if a and b:
            d=a['time_p50']-b['time_p50']; pct=d/a['time_p50']*100
            marker='>>' if d>0 else '<<'
            print(f'  {marker} REV1 delta vs V5.1: {-d:+.2f}s ({-pct:+.1f}%)   [negative = REV1 faster]')
            # consistency
            print(f'     time std  V5.1={a["time_std"]:.2f}s  REV1={b["time_std"]:.2f}s')
            # launch overhead delta
            if a['launch_overhead'] and b['launch_overhead']:
                print(f'     launch overhead  V5.1={a["launch_overhead"]:.2f}s  REV1={b["launch_overhead"]:.2f}s')
        print()
