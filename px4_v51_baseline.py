...
class V51Planner:
    def __init__(self, max_speed=12.0, cruise_speed=10.0, base_blend=2.5):
        ...
        self.px4_max_accel=6.0; self.px4_max_decel=4.0; self.px4_speed_ceiling=9.5  # promoted from 8.5 in Session 7 (ceil95_max12)
        self.px4_util_straight=0.78; self.px4_util_turn=0.55
        self.px4_hover_ramp_time=2.0
        self.last_cmd_speed=0.0; self.mission_start_time=None; self.cold_ramp_seed=0.0  # S8 prefill seed
        ...
        # S9 transition_blend: state-driven LAUNCH ramp
        self.tb_enabled=True; self.tb_vz_thresh=1.0
    ...
    def plan(self, pos, vel, target, next_gate=None):
        if self.mission_start_time is None:
            ...
            if self.cold_ramp_seed>0: self.last_cmd_speed=min(self.cold_ramp_seed,self.px4_speed_ceiling)
        ...
        if phase=='LAUNCH':
            ...
            if self.tb_enabled:
                time_frac=min(1.0, el/self.px4_hover_ramp_time)
                alt_now=abs(pos[2]); alt_tgt=abs(target[2])
                alt_frac=min(1.0, alt_now/max(alt_tgt, 0.1))
                vz_now=abs(vel[2]) if len(vel)>2 else 0.0
                vz_frac=max(0.0, min(1.0, 1.0 - vz_now/self.tb_vz_thresh))
...
async def run_trial(planner, gates, threshold=2.5, use_takeoff_gate=True, gate_alt_frac=0.95, gate_vz_max=0.3, gate_timeout=10.0):
    drone=System()
...
async def main():
    ...
    await asyncio.sleep(2)
    results=[]
    configs=[('V5.1', V51Planner, dict(max_speed=11.0, cruise_speed=9.0, base_blend=1.5))]
    ...
    with open('/Users/conradweeden/ai-grand-prix/logs/px4_v51_baseline.json','w') as f:
        json.dump(out,f,indent=2)
    print('Saved to logs/px4_v51_baseline.json')
