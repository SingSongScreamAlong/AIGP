"""
Mock-sim A/B: REV2 vs V5.1, in the same domain the execution_model curves were fit from.

REV2 = V5.1 + ASYMMETRIC Curve 1 (bin bonus only *raises* the flat baseline ceiling,
       never lowers it) + 0.5s ceiling-hold after gate pass. No launch changes.
after each gate pass to prevent the exit-rebuild loss observed in EXM1.
LAUNCH behavior is V5.1's (Curve 2 dropped — it was descriptive, not prescriptive).

Phase bucketing, launch delivery, sustain underdelivery, and consistency are computed
per-trial so we can see which loss buckets the curve revision actually moves.

Run:
  python3.13 mock_ab_rev2_vs_v51.py
"""
...
LOG_PATH     = os.path.join(BASE_DIR, "logs/mock_ab_rev2_vs_v51.json")
TRIALS_PER_COMBO = 3
MAVSDK_PORT  = 14540
...
class V51Planner:
    ...
    # Hook V5.1 uses scalar ceiling; REV2 overrides this to apply a bin BONUS.
    def _ceiling(self, state, target, dxy):
        return self.px4_speed_ceiling


# =====================================================================
# REV2 Planner — ASYMMETRIC Curve 1:
#   effective_ceiling(L) = max(baseline_flat_ceiling,
#                              fitted_bin_value(L) + MARGIN)
#
# The bin lookup can ONLY raise the ceiling above V5.1's flat baseline,
# never lower it. Short/mid legs where the fitted curve is pessimistic
# stay at V5.1 behavior; long legs where there is real unlock potential
# get extra headroom.
#
# Rev1 taught us that feeding fitted (descriptive) bin values directly
# as ceilings throttles V5.1's winning behavior, because most bins sit
# below 8.5. Rev2 keeps the V5.1 floor and adds a positive-only bonus.
#
# Smoothing: after each gate pass, hold MAX(prev_effective, new_effective)
# for HOLD_SECS to avoid an exit-rebuild penalty when crossing from a
# bonus-unlocked long leg back into a short leg.
# =====================================================================
class Rev2Planner(V51Planner):
    _CEILING_BINS = [
        (0,   8,  7.21),
        (8,  12,  7.81),
        (12, 16,  8.34),
    ...
    HOLD_SECS = 0.5
    MARGIN    = 0.5   # positive headroom added to fitted bin value
    ...
    def __init__(self, max_speed=11.0, cruise_speed=9.0, base_blend=1.5):
        ...
        self._gate_pass_time=None

    def _effective_ceiling(self, L):
        """Asymmetric: never below px4_speed_ceiling."""
        baseline = self.px4_speed_ceiling
        if L is None:
            return baseline
        for lo,hi,v in self._CEILING_BINS:
            if lo<=L<hi:
                return max(baseline, v + self.MARGIN)
        return baseline

    def _ceiling(self, state, target, dxy):
        new_c = self._effective_ceiling(self.current_leg_length)
        # Smoothing hold: keep max(prev, new) for HOLD_SECS after gate pass.
        if self._gate_pass_time is not None and self._prev_ceiling is not None:
            elapsed = time.time() - self._gate_pass_time
            if elapsed < self.HOLD_SECS:
                return max(self._prev_ceiling, new_c)
        return new_c

    def on_gate_passed(self, speed):
        # Snapshot effective ceiling we were *just* using before recomputing.
        self._prev_ceiling = self._effective_ceiling(self.current_leg_length)
        self._gate_pass_time = time.time()
        super().on_gate_passed(speed)
        self._new_leg = True

    def plan(self, state, target, next_gate=None):
        # Record leg length at the start of each new leg, then delegate
        # entirely to V5.1 (which calls self._ceiling for the bonus lookup).
        if self._new_leg:
            dx=target[0]-state.pos[0]; dy=target[1]-state.pos[1]
            self.current_leg_length=math.sqrt(dx*dx+dy*dy)
            self._new_leg=False
...
async def main():
    ...
    configs=[
        ...
        ('REV2', Rev2Planner, dict(max_speed=11.0, cruise_speed=9.0, base_blend=1.5)),
    ]
    ...
    # ---- Summary ----
    print(f'\n\n{"="*80}')
    print('REV2 vs V5.1 HEAD-TO-HEAD on MOCK SIM  (median over trials, OK only)')
    print(f'{"="*80}')
    ...
    for cn in ['mixed','sprint','technical']:
        ...
        a=summary.get((cn,'V5.1')); b=summary.get((cn,'REV2'))
        if a and b:
            d=a['time_p50']-b['time_p50']; pct=d/a['time_p50']*100
            marker='>>' if d>0 else '<<'
            print(f'  {marker} REV2 delta vs V5.1: {-d:+.2f}s ({-pct:+.1f}%)   [negative = REV2 faster]')
            # consistency
            print(f'     time std  V5.1={a["time_std"]:.2f}s  REV2={b["time_std"]:.2f}s')
            # launch overhead delta
            if a['launch_overhead'] and b['launch_overhead']:
                print(f'     launch overhead  V5.1={a["launch_overhead"]:.2f}s  REV2={b["launch_overhead"]:.2f}s')
        print()
