# ab_dirblend_s12.py — Session 12 Directional Exit Blend A/B.
# Control: current baseline (no directional blend).
# Treatment: post-turn directional blend — 0.25s linear interpolation from
#   achieved velocity tangent at gate pass toward planner pull vector.
...
RESULT = os.path.join(LOGS, 'ab_dirblend_s12.json')
N_TECH = 10
N_MIXED = 10
TRIAL_TIMEOUT = 200.0
...
BLEND_TA_THRESH = 0.524   # 30deg — trigger only on significant turns
BLEND_DURATION = 0.25     # seconds of directional interpolation

ARMS = [
    {'name': 'control_baseline', 'blend_enabled': False},
    {'name': 'treatment_dirblend', 'blend_enabled': True},
]

class V51WithDirBlend(B.V51Planner):
    """V51Planner subclass: directional exit blend after significant turns.
    Does NOT touch speed magnitude, smooth rates, util, or any scalar knob.
    Only rotates the commanded velocity direction during blend window."""
    def __init__(self, *args, blend_enabled=False, **kwargs):
        super().__init__(*args, **kwargs)
        self.blend_enabled = blend_enabled
        self.blend_active = False
        self.blend_start_time = 0.0
        self._pre_gate_ta = 0.0
        # Tangent direction at gate pass (unit vector, frozen)
        self.exit_tang_un = 0.0
        self.exit_tang_ue = 0.0

    def on_gate_passed(self, speed, vel_n=None, vel_e=None):
        """Extended to capture achieved velocity direction at gate pass."""
        super().on_gate_passed(speed)
        if self.blend_enabled and self._pre_gate_ta > BLEND_TA_THRESH:
            # Store achieved velocity direction as frozen unit vector
            if vel_n is not None and vel_e is not None:
                tang_spd = math.sqrt(vel_n * vel_n + vel_e * vel_e)
                if tang_spd > 0.1:
                    self.exit_tang_un = vel_n / tang_spd
                    self.exit_tang_ue = vel_e / tang_spd
    ...
    def plan(self, pos, vel, target, next_gate=None):
        # Compute turn angle for blend trigger (same as Session 11)
        ...
        if next_gate and dxy > 0.1:
            ...
            else: self._pre_gate_ta = 0.0
        else: self._pre_gate_ta = 0.0

        # Call parent plan for normal behavior (sets speed, direction, smoothing)
        result = super().plan(pos, vel, target, next_gate)

        # Check if blend window is still active
        if not (self.blend_enabled and self.blend_active):
            return result

        dt_since_gate = time.time() - self.blend_start_time
        if dt_since_gate >= BLEND_DURATION:
            self.blend_active = False
            return result

        # === DIRECTIONAL BLEND ===
        # alpha: 0 at gate pass (100% tangent), 1 at end (100% planner pull)
        alpha = min(1.0, dt_since_gate / BLEND_DURATION)

        # Parent's commanded velocity
        cmd_vn = result.north_m_s
        cmd_ve = result.east_m_s
        cmd_spd = math.sqrt(cmd_vn * cmd_vn + cmd_ve * cmd_ve)

        if cmd_spd < 0.01:
            return result

        # Pull direction (parent's intended direction, unit vector)
        pull_un = cmd_vn / cmd_spd
        pull_ue = cmd_ve / cmd_spd

        # Blend: (1-alpha)*frozen_tangent + alpha*pull_direction
        blend_un = (1.0 - alpha) * self.exit_tang_un + alpha * pull_un
        blend_ue = (1.0 - alpha) * self.exit_tang_ue + alpha * pull_ue

        # Normalize to unit vector
        blend_mag = math.sqrt(blend_un * blend_un + blend_ue * blend_ue)
        if blend_mag < 0.001:
            return result
        blend_un /= blend_mag
        blend_ue /= blend_mag

        # Scale to parent's speed magnitude (direction only, no speed change)
        new_vn = blend_un * cmd_spd
        new_ve = blend_ue * cmd_spd

        # Recompute yaw from blended direction
        new_yaw = math.degrees(math.atan2(new_ve, new_vn))

        return VelocityNedYaw(new_vn, new_ve, result.down_m_s, new_yaw)
...
async def run_one(course, arm):
    ...
    planner = V51WithDirBlend(max_speed=12.0, cruise_speed=9.0, base_blend=1.5,
                              blend_enabled=arm['blend_enabled'])
    ...
    try:
        ...
        while gi < len(gates):
            ...
            if d < 2.5:
                ...
                # Pass achieved velocity components for directional blend
                planner.on_gate_passed(gspd, vel_n=vel[0], vel_e=vel[1])
                gi += 1
                continue
...
async def main():
    bench.acquire_singleton('dirblend12')
    for k, v in (('MPC_XY_VEL_P_ACC',6.0),('MPC_ACC_HOR',10.0),
                 ('MPC_ACC_HOR_MAX',10.0),('MPC_JERK_AUTO',30.0)):
        B.RACE_PARAMS[k] = v
    print(f'[init] Session 12 Directional Exit Blend A/B', flush=True)
    print(f'[init] BLEND params: ta_thresh={BLEND_TA_THRESH:.3f}rad({math.degrees(BLEND_TA_THRESH):.0f}deg) '
          f'dur={BLEND_DURATION}s', flush=True)
    ...
    print(f'\n=== VERDICT: {final} Directional Exit Blend ===', flush=True)
    ...
    for course in ('technical', 'mixed'):
        ...
        for t in trials:
            ...
            rt = [r for r in results if r['course']==course and r['trial']==t and r['arm']=='treatment_dirblend']
