...
**Status as of 2026-04-28 (post-attitude-controller):** Adapter seam exists (`src/sim/adapter.py::DCLSimAdapter`). Capability flags updated per DCL tech spec: `ATTITUDE | CAMERA_RGB | IMU | RESET` — no velocity or position commands. `AttitudeController` (`src/control/attitude_controller.py`) converts velocity NED → T/R/P/Y. `RaceLoop` auto-detects ATTITUDE capability and wires the controller. `MockDCLAdapter` now models attitude dynamics (thrust tilt + drag + yaw tracking). Pre-landing scaffolding complete: `MockDCLAdapter` + `test_dcl_smoke.py` + `scripts/day1_dcl.sh`.
...
4. **Attitude path (DCL):** `await adapter.send_attitude(roll, pitch, yaw, throttle)` — via `AttitudeController` converting velocity NED → T/R/P/Y
   **Velocity path (PX4):** `await adapter.send_velocity_ned(vn, ve, vd, yaw_deg)` — planner output direct
...
| `send_attitude()` | **hot** | DCL tech spec: Throttle/Roll/Pitch/Yaw | RaceLoop auto-wires AttitudeController when adapter has ATTITUDE cap |
...
| `send_velocity_ned()` | N/A | DCL does not support velocity commands | AttitudeController handles translation |
| `send_position_ned()` | N/A | DCL does not support position commands | Not needed |
...
- [ ] **Wire `send_attitude()`.** Map `(roll_deg, pitch_deg, yaw_deg, thrust)` to DCL's stick API. Confirm sign conventions (roll right = positive? pitch forward = negative?). thrust ∈ [0, 1].
- [ ] **Run `python3 run_race.py --backend dcl --detector virtual --course technical --timeout 30`.** Virtual detector works without a camera — it isolates the adapter. RaceLoop auto-creates AttitudeController when it detects ATTITUDE capability. If this completes, the state/command seam is good.
...
6. **`command_hz` matches DCL's tick rate.** Default is 50 Hz. If DCL's physics is 120 Hz (matching the spec), the race loop should still work since `send_attitude` is rate-limited at the loop layer, but if DCL is e.g. 30 Hz and the loop fires 50 Hz, commands get dropped silently. Worth logging the first few tick timings to confirm.
7. **AttitudeController PID gains.** Defaults are conservative (tuned on `MockDCLAdapter` dynamics). Real DCL drone will have different mass/inertia/drag. Plan to re-tune gains on day 1 using step-response tests.
...
- [x] `MockDCLAdapter` (`src/sim/mock_dcl.py`) models attitude dynamics (thrust-tilt + drag + yaw tracking). Capabilities: `ATTITUDE | CAMERA_RGB | IMU | RESET`. Wired into `run_race.py` as `--backend mock_dcl`.
- [x] `AttitudeController` (`src/control/attitude_controller.py`) — PID cascade converting velocity NED → T/R/P/Y. Auto-wired by `RaceLoop` when adapter has `ATTITUDE` but not `VELOCITY_NED`. 23 unit tests + 21 closed-loop integration tests passing.
