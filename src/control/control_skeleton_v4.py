...
class Planner:
    """V4: PX4-aware velocity planner.
    
    Key changes from V3:
    - PX4 response model: commands what PX4 can deliver, not ideal kinematics
    - Segment-aware speed: accounts for leg length vs acceleration capability
    - Hover-start ramp: special first-gate handling
    - Achievable-speed targeting: inverts utilization curve
    - Smoother command transitions for better PX4 tracking
    """
    ...
    def __init__(self, max_speed=11.0, cruise_speed=9.0, base_blend=1.5):
        ...
        # PX4 response model (empirical from benchmark data)
        self.px4_max_accel = 6.0       # realistic achieved accel m/s^2 (not commanded 10)
        self.px4_speed_ceiling = 8.0   # practical max achieved speed on PX4 SIH
        self.px4_util_straight = 0.75  # straight-line cmd->achieved ratio
        self.px4_util_turn = 0.55      # turning cmd->achieved ratio
        self.px4_hover_ramp_time = 3.0 # seconds to reach usable speed from hover
        
        # State tracking for smoother commands
        self.last_cmd_speed = 0.0
        self.mission_start_time = None
        self.gates_passed = 0
    
    def _px4_achievable_speed(self, raw_speed, turn_severity, segment_length, current_speed):
        """Predict what speed PX4 will actually achieve given a command.
        
        Instead of commanding raw desired and getting 66% of it,
        command an adjusted value so PX4 delivers closer to our actual target.
        """
        # Utilization varies with turn severity (0=straight, pi=U-turn)
        turn_ratio = turn_severity / math.pi  # 0..1
        util = self.px4_util_straight * (1 - turn_ratio) + self.px4_util_turn * turn_ratio
        
        # Segment length penalty: short legs can't reach cruise
        # Time to accelerate from current speed to target:
        # t_accel = (target - current) / accel
        # Distance consumed: d = current*t + 0.5*accel*t^2
        # If segment is shorter than that, cap the target
        if segment_length > 0.1:
            max_achievable = math.sqrt(current_speed**2 + 2 * self.px4_max_accel * segment_length)
            max_achievable = min(max_achievable, self.px4_speed_ceiling)
            raw_speed = min(raw_speed, max_achievable)
        
        # Invert utilization: if we want PX4 to achieve X, command X/util
        # But cap at ceiling to avoid absurd commands
        commanded = min(raw_speed / max(util, 0.3), self.max_speed)
        
        return commanded, raw_speed * util  # (what to command, expected achieved)
    
    def _smooth_speed(self, target_speed, dt=0.02):
        """Rate-limit speed commands to avoid step changes PX4 can't follow."""
        max_speed_rate = 8.0  # m/s per second - how fast we change speed commands
        max_delta = max_speed_rate * dt
        
        if target_speed > self.last_cmd_speed:
            smoothed = min(target_speed, self.last_cmd_speed + max_delta)
        else:
            # Allow faster deceleration commands (PX4 decelerates more willingly)
            smoothed = max(target_speed, self.last_cmd_speed - max_delta * 2.0)
        
        self.last_cmd_speed = smoothed
        return smoothed
    
    def plan_velocity(self, state, target_gate, next_gate=None):
        """V4 velocity-mode planning - PX4-aware."""
        if target_gate is None:
            return VelocityNedYaw(0, 0, 0, 0)
        
        # Track mission time for hover-start handling
        ...
        # ── Dynamic blend radius (V3 base + PX4-aware adjustments) ──
        speed_factor = 0.25  # slightly increased for PX4 lag
        turn_factor = turn_angle / math.pi
        dynamic_blend = self.base_blend + speed_factor * current_speed + turn_factor * 2.0
        self.blend_radius = dynamic_blend
        
        # ── PX4-aware speed selection ──
        
        # Base: turn-angle-aware cruise (V3 logic, retained)
        cos_factor = math.cos(turn_angle / 2.0)
        turn_speed_mult = 0.4 + 0.6 * cos_factor
        base_target = self.cruise_speed * turn_speed_mult
        
        # Hover-start ramp: first few seconds, progressively increase
        mission_elapsed = time.time() - self.mission_start_time
        if mission_elapsed < self.px4_hover_ramp_time:
            ramp = mission_elapsed / self.px4_hover_ramp_time
            ramp = ramp * ramp  # quadratic ramp for smooth start
            base_target *= ramp
            base_target = max(base_target, 2.0)  # minimum forward speed
        
        # Segment-length-aware: if remaining distance is short, don't overshoot
        if dist_xy < 8.0:
            # Scale down if we're close and need to turn
            proximity_factor = dist_xy / 8.0
            if turn_angle > 0.5:  # meaningful turn ahead
                base_target *= (0.5 + 0.5 * proximity_factor)
        
        # Get PX4-achievable command
        commanded_speed, expected_achieved = self._px4_achievable_speed(
            base_target, turn_angle, dist_xy, current_speed
        )
        
        # ── Blend zone (entering turn toward next gate) ──
        if next_gate is not None and dist_xy < dynamic_blend:
            ...
            # Apex slowdown - V4: scaled by how hard PX4 finds this turn
            # Harder turns (higher turn_angle) get more slowdown
            apex_depth = 0.2 + 0.2 * (turn_angle / math.pi)  # 0.2 to 0.4
            apex_factor = 1.0 - apex_depth * math.sin(blend * math.pi)
            speed = commanded_speed * apex_factor
            ...
            # ── Pre-blend zone: anticipatory deceleration ──
            if dist_xy < dynamic_blend * 2.0 and turn_angle > 0.3:
                # V4: start shaping earlier than V3, based on PX4 decel rate
                # PX4 needs more lead time to change velocity
                approach_ratio = dist_xy / (dynamic_blend * 2.0)
                decel_factor = 0.6 + 0.4 * approach_ratio  # 0.6 at blend edge, 1.0 far
                speed = commanded_speed * decel_factor
            else:
    ...
    def plan_position(self, state, target_gate, next_gate=None):
        """Position-mode planning - safe/precise. Unchanged from V3."""
        if target_gate is None:
...
async def main():
    ...
    print("  V4: PX4-Aware Adaptive Planner")
    print("=" * 60)
    print()
