...
class Planner:
    """Generates velocity targets to fly through gates at speed.

    Uses a blend strategy:
    - Far from gate: full speed toward gate
    - Near gate: blend velocity toward next gate (pure-pursuit style)
    - At gate: already moving toward next gate, no deceleration

    Modes:
    - "velocity": VelocityNedYaw commands (smooth racing lines)
    - "position": PositionNedYaw commands (safe hover/precision)
    """

    def __init__(self, max_speed=5.0, cruise_speed=4.0, blend_radius=2.5):
        self.max_speed = max_speed      # absolute max m/s
        self.cruise_speed = cruise_speed  # target cruise m/s
        self.blend_radius = blend_radius  # meters — start blending toward next gate
        self.mode = "velocity"           # "velocity" or "position"

    def plan_velocity(self, state, target_gate, next_gate=None):
        """Velocity-mode planning — smooth racing lines through gates.

        Returns (VelocityNedYaw, yaw_deg) for offboard velocity control.
        """
        ...
        if dist_3d < 0.1:
            # On top of gate — just hold
            return VelocityNedYaw(0, 0, 0, yaw_deg)

        # Unit vector toward current gate
        ux, uy, uz = dx/dist_3d, dy/dist_3d, dz/dist_3d

        # Determine speed profile
        if next_gate is not None and dist_xy < self.blend_radius:
            # BLEND ZONE: smoothly transition velocity toward next gate
            # Blend factor: 1.0 at gate, 0.0 at blend_radius edge
            blend = 1.0 - (dist_xy / self.blend_radius)
            ...
            # Blend direction: current gate → next gate
            ...
            # Maintain cruise speed through the blend
            speed = self.cruise_speed
            vx = bx * speed
            vy = by * speed
            vz = bz * speed

            # Update yaw to blend direction
            yaw_deg = math.degrees(math.atan2(by, bx))

        else:
            # APPROACH: full speed toward gate
            # Speed ramps up quickly, caps at cruise
            speed = min(self.cruise_speed, max(1.8, dist_xy * 2.4))
            vx = ux * speed
            vy = uy * speed
            vz = uz * speed

        # Altitude correction — always gently push toward gate altitude
        alt_err = tz - state.pos[2]
        vz = alt_err * 3.0  # P-gain for altitude
    ...
    def plan_position(self, state, target_gate, next_gate=None):
        """Position-mode planning — safe/precise. Original behavior."""
        if target_gate is None:
...
class Controller:
    ...
    def __init__(self, connection_string="udpin://0.0.0.0:14540"):
        ...
        self.planner = Planner(max_speed=8.0, cruise_speed=6.0, blend_radius=0.8)
        self.command_hz = 50
...
async def main():
    ...
    print("  Phase 0: MAVSDK + Telemetry + Position Control")
    print("=" * 60)
    print()
