...
class Planner:
    ...
    def __init__(self, max_speed=11.0, cruise_speed=9.0, base_blend=1.5):
        ...
        self.base_blend = base_blend     # minimum blend radius
        self.blend_radius = base_blend   # will be updated dynamically
        self.mode = "velocity"           # "velocity" or "position"

    def plan_velocity(self, state, target_gate, next_gate=None):
        """V3 velocity-mode planning - adaptive blend + cornering."""
        ...
        # Compute turn angle to next gate
        turn_angle = 0.0
        ...
        dynamic_blend = self.base_blend + speed_factor * current_speed + turn_factor * 1.5
        self.blend_radius = dynamic_blend

        # Turn-angle-aware speed
        cos_factor = math.cos(turn_angle / 2.0)
        turn_speed_mult = 0.4 + 0.6 * cos_factor
        adjusted_cruise = self.cruise_speed * turn_speed_mult
        ...
        if next_gate is not None and dist_xy < dynamic_blend:
            ...
            if dist_xy < dynamic_blend * 1.5:
                approach_factor = dist_xy / (dynamic_blend * 1.5)
                speed = adjusted_cruise * (0.7 + 0.3 * approach_factor)
            else:
                speed = min(adjusted_cruise, max(1.8, dist_xy * 2.4))
...
async def main():
    ...
    print("  V3: Adaptive Blend + Cornering Planner")
    print("=" * 60)
    print()
