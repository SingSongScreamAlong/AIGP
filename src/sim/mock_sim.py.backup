...
class MockDrone:
    ...
    def __init__(self):
        ...
        # ── V2 physics constraints ──────────────────────
        self.max_accel = 20.0        # m/s² (~2g)
        self.max_turn_rate = 4.0     # rad/s max heading change
        self.cmd_latency = 0.02      # seconds command pipeline delay
        self.heading = 0.0           # current heading (rad)
        self._cmd_buffer = []        # [(apply_time, vel_target)]
        self._effective_vel_target = None
    ...
    def step(self, dt):
        ...
        if self.control_mode == "velocity" and self.vel_target is not None:
            ...
            # ── V2: Command latency ──
            now = time.time()
            if not self._cmd_buffer or not np.allclose(self._cmd_buffer[-1][1], self.vel_target, atol=0.01):
                self._cmd_buffer.append((now + self.cmd_latency, self.vel_target.copy()))
            while self._cmd_buffer and self._cmd_buffer[0][0] <= now:
                _, cmd = self._cmd_buffer.pop(0)
                self._effective_vel_target = cmd
            if self._effective_vel_target is None:
                self._effective_vel_target = self.vel_target.copy()

            vel_cmd = self._effective_vel_target.copy()
            speed_cmd = np.linalg.norm(vel_cmd)
            if speed_cmd > self.max_speed:
                vel_cmd = vel_cmd / speed_cmd * self.max_speed
                speed_cmd = self.max_speed

            # ── V2: Heading-rate-limited direction tracking ──
            if speed_cmd > 0.1:
                desired_heading = math.atan2(vel_cmd[1], vel_cmd[0])
            else:
                desired_heading = self.heading
            heading_err = desired_heading - self.heading
            heading_err = (heading_err + math.pi) % (2 * math.pi) - math.pi
            max_dh = self.max_turn_rate * dt
            if abs(heading_err) > max_dh:
                heading_err = math.copysign(max_dh, heading_err)
            self.heading += heading_err

            # ── V2: Heading-dependent thrust projection ──
            lateral_authority = 0.5
            h_vec = np.array([math.cos(self.heading), math.sin(self.heading), 0.0])
            vel_cmd_xy = np.array([vel_cmd[0], vel_cmd[1], 0.0])
            along = np.dot(vel_cmd_xy, h_vec)
            cross_vec = vel_cmd_xy - along * h_vec
            cross_mag = np.linalg.norm(cross_vec)
            if cross_mag > 0.01:
                eff_xy = along * h_vec + cross_vec * lateral_authority
            else:
                eff_xy = along * h_vec
            eff_cmd = np.array([eff_xy[0], eff_xy[1], vel_cmd[2]])

            # ── V2: Bounded acceleration ──
            vel_err = eff_cmd - self.vel
            accel = 8.0 * vel_err
            ...
            # Derive attitude from actual velocity
