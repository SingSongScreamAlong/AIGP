# ab_soak.py — Session 9 bench-hardening soak.
# 20-trial control-only run on the locked baseline (V5.1 + ceil95_max12).
# All interventions OFF. Purpose: certify the bench, not test a hypothesis.
...
RESULT = os.path.join(LOGS, 'soak_s9.json')
N_SOAK = 20
COURSE = 'mixed'  # primary surface; if mixed completes cleanly the bench is good
TRIAL_TIMEOUT = 200.0
POST_RESTART_WAIT = 2.0  # bench.hardened_restart already waits for "Ready for takeoff"

async def run_one(course):
    """Run a single baseline trial. No intervention knobs touched."""
    drone = System()
    ...
    planner.z_gate_alt_frac = 0.0  # explicitly OFF
    ...
    try:
        ...
        # always tear down telemetry task — fixes leak that bled across trials
        stop_pl.set()
        pl_task.cancel()
        try:
            await pl_task
...
async def trial(idx, results):
    ...
    try:
        ...
        r = {'trial': idx, 'completed': False, 'error': f'restart_failed: {e}'}
        results.append(r)
...
async def main():
    ...
    # locked tune (Session 7 baseline)
    ...
    # PREFLIGHT: 1 trial. Abort sweep if it fails.
    ...
    print('\n  per-trial:')
    for r in soak:
        print(f'    t{r["trial"]:>2} lap={r.get("lap_time")} done={r.get("completed")} err={r.get("error")}')
