"""V5 vs V4 Head-to-Head on PX4 SITL"""
...
class V5Planner:
    def __init__(self, max_speed=12.0, cruise_speed=10.0, base_blend=2.5):
        ...
        self.px4_hover_ramp_time=2.5
        self.last_cmd_speed=0.0; self.mission_start_time=None
        self.gates_passed=0; self.is_first_leg=True; self.prev_gate_speed=0.0
    def _turn_entry_speed(self, ta):
        return min(self.cruise_speed*(0.35+0.65*math.cos(ta/2)), self.px4_speed_ceiling)
    def _decel_distance(self, cs, ta):
        ...
        return max(d*1.3, self.base_blend*1.5)
    def _phase(self, dxy, cs, ta, db, seg_len):
        ...
        if seg_len<10.0: return 'SHORT'
        if cs<self.cruise_speed*0.7: return 'BUILD'
        return 'SUSTAIN'
    ...
    def _smooth(self, target, phase, dt=0.02):
        rates={'LAUNCH':4.0,'BUILD':10.0,'PRE_TURN':12.0,'TURN':6.0,'SHORT':8.0,'SUSTAIN':8.0}
    ...
    def plan(self, pos, vel, target, next_gate=None):
        ...
        phase=self._phase(dxy,cs,ta,db,dxy)
        if phase=='LAUNCH':
            el=time.time()-self.mission_start_time
            desired=max(self.cruise_speed*((el/self.px4_hover_ramp_time)**2),2.0)
        elif phase=='SHORT':
            if ta>1.0: desired=min(cs,self.cruise_speed*0.5)
            else: desired=min(cs+1.0,self.cruise_speed*0.7)
            ...
            ad=0.2+0.25*(ta/math.pi); af=1.0-ad*math.sin(bl*math.pi)
            desired=te*af
...
async def main():
    ...
    for cn in ['sprint','technical','mixed']:
        ...
        print(f'{"="*60}')
        for vname,PlannerCls in [('V4',V4Planner),('V5',V5Planner)]:
    ...
    for cn in ['sprint','technical','mixed']:
        ...
        if len(cr)==2:
            v4t=cr[0]['time']; v5t=cr[1]['time']
            delta=v4t-v5t; pct=delta/v4t*100
            print(f'  >> V5 delta: {delta:+.1f}s ({pct:+.1f}%)')
        print()

    with open('/Users/conradweeden/ai-grand-prix/logs/v5_vs_v4.json','w') as f:
        json.dump(results,f,indent=2)
    print('Saved to logs/v5_vs_v4.json')
